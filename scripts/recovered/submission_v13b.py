"""SUBMISSION V13B — private-round carrier: era gated at h>=4 (v13 LB feedback applied).

Board after v13 LB feedback:
  v12b 0.695357171 (BEST) | v13 0.696325144 (h3-era = +0.003 damage on test; val said -0.003
  — noise-level sign flip, but the pre-registered rule fires: gate moves to h>=4).

Test structure (verified): public masked rows are ALL h2/h3 (31,151 + 31,132); private
masked rows = h2 31,189 / h3 15,591 / h4 31,115 / h5 15,617 / h6 15,568 / h7 15,550.
  => gate h>=4: ZERO public rows touched -> public bit-identical to v12b = 0.695357171 EXACT.
  => private h4-h7 (77,850 rows = 62.5% of private masked) get era Dtil (val: -0.03/-0.04).

Structure (bit-exact by construction):
  v13b k0 rows        == v12b k0 rows      (same build_k0B(Dtil_era))
  v13b ALL public rows== v12b public rows  (public masked = h2/h3 only -> static pred)
  v13b private h>=4   == v12a masked rows  (era pred)
This CSV exists to be SELECTED as private submission #1 (Zindi: choose 2 for private LB).
"""
...
TAU = 12.0
H_ERA_MIN = 4   # era Dtil applied to masked pred rows with h >= 4 (v13 rule: h3 moved to static)
...
# ---- v13 ----
print("\n--- v13b: k0-era + masked gated-era h>=4 (private carrier, public == v12b) ---", flush=True)
assemble(p_ens_gated, k0B_era, 'v13b')
...
o = pd.read_csv(f'{DL}/submission_v13b.csv')
assert len(o) == 280961
...
# structure check: v13b must equal v12b on ALL public rows + private h2/h3; equal v12a on h>=4
try:
    ...
    same_h23 = np.abs(v13[m_h23] - v12b[m_h23])
    same_h4 = np.abs(v13[m_h4p] - v12a[m_h4p])
    print(f"\n  STRUCTURE: k0    vs v12b  max|d|={same_k0.max():.6f} (LGBM jitter exp. <0.11)")
    print(f"  STRUCTURE: h2+h3 vs v12b  max|d|={same_h23.max():.6f} (expect ~0)")
    print(f"  STRUCTURE: h>=4  vs v12a  max|d|={same_h4.max():.6f} (expect ~0)")
    print(f"  STRUCTURE: public rows vs v12b max|d|={np.abs(v13[pub]-v12b[pub]).max():.6f} (expect 0 -> public = 0.695357171 EXACT)")
except FileNotFoundError as e:
    print(f"  structure check skipped: {e}")

print(f"\n  range [{v13.min():.3f}, {v13.max():.3f}]")
print(f"  PRE-REGISTERED public prediction: 0.695357171 EXACTLY (public bit-identical to v12b)")
print(f"  ROLE: private-round carrier #1 (era on h4-h7 = 62.5% of private masked rows). SELECT IT before Sep 13.")
print("\nDONE.")

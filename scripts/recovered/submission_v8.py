"""
SUBMISSION V8 — final build.

Validated components (see scripts/v8_cv.py, v8_loo2.py, fresh_eyes_val.py):
  k=0 rows (33.5%): two-component D-aware model (val 0.6286 -> 0.5867 linear,
             0.5827 blended with LGBM) — test anomaly = D (persists ~1) + FAST (persists ~0.6-0.7)
  masked rows (66.5%): v2b-core Kalman + obs-denoise + per-cell H/R era-scaled
             (test-distance LOO: 0.7439 -> 0.7369); shrink 0.85 only in hedge (CV says helps,
             true-distance LOO says hurts -> arbitrate on LB)

Variants:
  v8a: masked [obs-dn + percell H/R], k0 [0.5 linear + 0.5 LGBM]   <- primary
  v8b: masked [obs-dn + percell H/R + shrink 0.85], k0 [blend]      <- shrink hedge
  v8c: masked [obs-dn], k0 [linear two-comp only]                   <- minimal risk
"""
...
print("\nAll 3 submissions verified: 280,961 rows, IDs match, no NaN.")
print("DONE.")

"""SUBMISSION V15 — the private-round carrier: v14's PUBLIC-VALIDATED k0 lane + era h>=4 gate.

Board after v14 LB feedback:
  v14  0.694044494 NEW BEST (k0-new recipe CONFIRMED: implied public k0 0.5871 -> 0.5834;
        landed ABOVE pre-registered band 0.6905-0.6925 -> val k0 gains transfer at ~30%).
  v12b 0.695357171 | v13 0.696325144 (h3-era sign-flipped) | v13b built (carrier, v12b-k0 + era h>=4).
  Pre-registered rule landed in the MIDDLE ZONE (0.6930 < 0.6940 < 0.6954):
  k0-new is real -> KEEP it; era h>=4 remains the private payload -> still gate it in.

v15 = v14 verbatim (k0 rows: 0.6*LGBM_only + 0.4*Kalman_h1(era->era), GAU10) + masked pred
      Dtil gated at h>=4 (era) instead of static-everywhere. Disjoint-row merge of the two
      proven-positive axes:
        k0 rows            == v14 k0 rows      (bit-exact: same recipe/seed)
        masked h2/h3 (ALL public masked + private h2/h3) == static == v10b/v12b/v14
        masked h>=4 (private only: 77,850 rows = 62.5% of private masked) == v12a era

PUBLIC SCORE: EXACTLY 0.694044494 (all public rows bit-identical to v14; public masked
  rows are h2/h3 only -> untouched by the h>=4 gate). ZERO public risk/information cost.
ROLE: private-round carrier #1 — combines the public-validated k0 gain with the largest
  val-validated private lever (era at h4/h5: -0.039/-0.037 per-row class gains, 10x noise).
  Supersedes v13b as primary selection; v13b remains fallback (old-k0 + era).
"""
...
TAU = 12.0
H_ERA_MIN = 4   # v15: era Dtil on masked pred rows with h >= 4 (ALL private; public masked = h2/h3 only)
...
def build_k0B(dtil):
    ...
    return k0_B_lin, k0_B_lgb, k0_blendB

def kalman_h1_k0(dtil_init, dtil_pred):
    """h=1 Kalman prediction for unmasked (k0) rows: x0 from own observation,
    phi-ens {0.74,0.80}, W(t+1) observation update with FROZEN H/R. Mirrors the
    val experiment's M1 (era->era) exactly."""
    Hc, Rc, var_fc = calibrate(W_pool)
    P0c = LAM_F*(1-LAM_F)*var_fc
    ta_m = mgt['t_abs'].values; cct_m = mgt['cc'].values
    tws_m = mgt['TWS_t'].values
    ok_rows = (~msk) & np.isfinite(tws_m)
    dcache = {}
    def dc(t):
        if t not in dcache: dcache[t] = dtil_pred(t)
        return dcache[t]
    dcache_i = {}
    def dci(t):
        if t not in dcache_i: dcache_i[t] = dtil_init(t)
        return dcache_i[t]
    pred = np.zeros(len(test), dtype=np.float64)
    for phi_f in [0.74, 0.80]:
        q1 = LAM_F*var_fc*(1-phi_f**2)
        P1 = phi_f**2*P0c + q1
        K1 = P1*Hc/(Hc*Hc*P1 + Rc)
        p = np.full(len(test), 0.0, dtype=np.float64)
        for m in np.unique(ta_m[ok_rows]):
            selm = ok_rows & (ta_m == m)
            cc_m = cct_m[selm]
            dt0 = dci(int(m))[cc_m]
            x0 = LAM_F*(tws_m[selm] - mu_c[cc_m] - dt0)
            x1 = phi_f*x0
            wv = W_pool.get(int(m)+1)
            if wv is not None:
                w = wv[cc_m]; okw = np.isfinite(w)
                x1 = np.where(okw, x1 + K1*(w - Hc*x1), x1)
            dt1 = dc(int(m)+1)[cc_m]
            p[selm] = mu_c[cc_m] + dt1 + x1
        pred += p/2.0
    pred[~ok_rows] = np.nan
    return pred
...
# ---- k0 v14: 0.6*LGBM-only + 0.4*Kalman h1 (era->era), GAU10-smoothed ----
print("\nk0-B components (era-inclusive Dcache)...", flush=True)
lin_era, lgb_era, blend_era = build_k0B(Dtil_era)
print("k0 Kalman h1 (era->era, frozen H/R, phi-ens)...", flush=True)
kal_era = kalman_h1_k0(Dtil_era, Dtil_era)
W_KAL = 0.40
k0_raw = (1.0-W_KAL)*np.nan_to_num(lgb_era) + W_KAL*np.nan_to_num(kal_era)
badK = (~msk) & ~np.isfinite(lgb_era) & ~np.isfinite(kal_era)
if badK.any(): k0_raw[badK] = mu_c[cc_t][badK]
k0B_v14 = smooth_rows(np.where(np.isfinite(k0_raw), k0_raw, mu_c[cc_t]), ~msk & tw_ok, GAU10)
k0B_v14 = np.where(np.isfinite(k0B_v14), k0B_v14, k0_raw)
print(f"k0 v14 blend: {(1.0-W_KAL):.2f}*lgb + {W_KAL:.2f}*kalman (era Dcache)")

# ---- v15 ----
print("\n--- v15: v14 k0 lane (public-validated) + masked era gated h>=4 (private carrier) ---", flush=True)
assemble(p_ens_gated, k0B_v14, 'v15')
...
for ref_tag in ['v14', 'v12b', 'v12a', 'v10b']:
    ...
    d = v15 - ref
    print(f"\n  v15 vs {ref_tag}: corr={np.corrcoef(v15, ref)[0,1]:.6f}")
...
# structure check (bit-exact by construction):
#   ALL public rows == v14  -> public = 0.694044494 EXACT
#   ALL k0 rows     == v14  -> public-validated k0 lane carried
#   masked h2/h3    == v12b/v10b (static)
#   masked h>=4     == v12a  (era)
try:
    v14r  = pd.read_csv(f'{DL}/submission_v14.csv')['Target'].values.astype(np.float64)
    ...
    m_h23 = msk & (h_row >= 2) & (h_row <= 3); m_h4p = msk & (h_row >= 4)
    print(f"\n  STRUCTURE: ALL public rows vs v14 max|d|={np.abs(v15[pub]-v14r[pub]).max():.6f} (expect 0 -> public = 0.694044494 EXACT)")
    print(f"  STRUCTURE: k0 rows vs v14      max|d|={np.abs(v15[unm]-v14r[unm]).max():.6f} (expect 0)")
    print(f"  STRUCTURE: masked h2/h3 vs v12b max|d|={np.abs(v15[m_h23]-v12b[m_h23]).max():.6f} (expect ~0, static)")
    print(f"  STRUCTURE: masked h>=4 vs v12a  max|d|={np.abs(v15[m_h4p]-v12a[m_h4p]).max():.6f} (expect ~0, era)")
    print(f"  STRUCTURE: n(h>=4 rows)={int(m_h4p.sum())} public-h>=4={int((m_h4p & pub).sum())} (expect 0 public)")
except FileNotFoundError as e:
    print(f"  structure check skipped: {e}")

print(f"\n  range [{v15.min():.3f}, {v15.max():.3f}]")
print(f"  PRE-REGISTERED public prediction: 0.694044494 EXACTLY (public bit-identical to v14)")
print(f"  ROLE: private-round carrier #1 (k0-new + era h>=4). SELECT 2 finals before Sep 13.")
print("\nDONE.")

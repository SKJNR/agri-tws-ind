...
for s in starts:
    ...
    if e + gap > t_max:
        continue
    a1 = piv_tws.iloc[sample_cells][range(s, e + 1)]
    a2 = piv_tws.iloc[sample_cells][range(e + 1, e + gap + 1)]
    d_target = a2.mean(axis=1).values - a1.mean(axis=1).values
    cum = piv_cov['SPEI_12_t'].iloc[sample_cells][range(s + 1, e + gap + 1)].mean(axis=1).values

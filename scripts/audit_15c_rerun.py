"""
AUDIT 15-c part 6: INSTRUMENTED RERUN of submission_v8.py
Goals:
  1. Count rows where the NaN-fallback (pred = mu_c) actually fired (per variant).
  2. Verify live use_full/use_red k0 counts.
  3. Reproducibility: rebuild v8a/b/c to scripts/tmp_audit/ and compare exactly
     against download/submission_v8{a,b,c}.csv.
Identical math to submission_v8.py; only instrumentation + output path differ.
"""
import numpy as np, pandas as pd, os

DATA = '/home/z/my-project/data'
DL = '/home/z/my-project/scripts/tmp_audit'
...
for a in anchors:
    ...
    tx = trendex(a)
    ok = np.isfinite(dloo)&np.isfinite(S)&np.isfinite(AF[a])&isinstance(tx, np.ndarray)
...
print(f"cells with NaN Dhat: {int(np.isnan(Dhat).sum())}; masked rows in those cells: {int(np.isnan(Dhat)[cc_t[msk]].sum())}")
...
print(f"k0 linear coef[FAST_est]={coefF[0]:.4f}")
print(f"[AUDIT] train k0 rows: full={int(selF.sum()):,} reduced={int((~selF).sum()):,}")
...
print(f"[AUDIT] test k0 rows: use_full={int(use_full.sum()):,} use_red={int(use_red.sum()):,} "
      f"unmasked_neither={int(((~msk)&~tw_ok).sum())} (of {int((~msk).sum()):,} unmasked)")
...
k0_lin[use_full] = mu_c[cct[use_full]] + Dt1[use_full] + Xv[use_full] @ coefF
k0_lin[use_red]  = mu_c[cct[use_red]]  + Dt1[use_red]  + Xv[use_red][:,colsR] @ coefR
k0_lgb = np.full(len(test), np.nan, dtype=np.float64)
k0_lgb[use_full] = mu_c[cct[use_full]] + Dt1[use_full] + bst.predict(XLv[use_full])
k0_lgb[use_red]  = mu_c[cct[use_red]]  + Dt1[use_red]  + bst_r.predict(XLv[use_red])
print(f"[AUDIT] k0_lin NaN at unmasked rows: {int(np.isnan(k0_lin[~msk]).sum())} "
      f"(rows neither full nor red: {int(((~msk)&~use_full&~use_red).sum())})")
k0_blend = 0.5*np.nan_to_num(k0_lin) + 0.5*np.nan_to_num(k0_lgb)
...
def masked_predict(obs_dn=True, percell=True, shrink=1.0):
    ...
    for m in sorted(set(ta[msk].tolist())):
        ...
        pred[sel] = mu_c[cc_t[sel]] + Dtil(tm)[cc_t[sel]] + shrink*x[cc_t[sel]]
    return pred
...
def assemble(masked_pred, k0_pred, tag):
    ...
    n_nan_before = int(np.isnan(pred).sum())
    nan_rows = np.where(np.isnan(pred))[0]
    nan_masked = int((nan_rows.size>0) and msk[nan_rows].sum())
    ...
    print(f"[AUDIT] {tag}: NaN before fallback = {n_nan_before} (masked: {nan_masked}); "
          f"fallback-to-mu_c fired on {n_nan_before} rows", flush=True)
    print(f"saved {tag}: mean={pred.mean():.4f} std={pred.std():.4f} NaN-free={ok}", flush=True)
    return pred

print("\n--- v8a: obs-dn + percell | k0 blend ---")
mp_a = masked_predict(obs_dn=True, percell=True, shrink=1.0)
p_a = assemble(mp_a, k0_blend, 'v8a')
print("--- v8b: obs-dn + percell + shrink 0.85 | k0 blend ---")
mp_b = masked_predict(obs_dn=True, percell=True, shrink=0.85)
p_b = assemble(mp_b, k0_blend, 'v8b')
print("--- v8c: obs-dn | k0 linear two-comp ---")
mp_c = masked_predict(obs_dn=True, percell=False, shrink=1.0)
p_c = assemble(mp_c, k0_lin, 'v8c')

# ================= reproducibility check =================
print("\n=== reproducibility vs download/ ===")
for tag in ['v8a','v8b','v8c']:
    new = pd.read_csv(f'{DL}/submission_{tag}.csv')['Target'].values
    old = pd.read_csv(f'/home/z/my-project/download/submission_{tag}.csv')['Target'].values
    d = np.abs(new - old)
    print(f"{tag}: max|new-old| = {d.max():.3e}  exact = {bool((new==old).all())}  n_diff_rows = {int((new!=old).sum())}")

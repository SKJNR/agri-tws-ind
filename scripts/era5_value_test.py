...
# anchor fields (dict per anchor month - memory-safe, no time x cell matrix)
val_cc = val['cc'].values
val_tws = val['TWS_t'].values
AF = {}
for a in sparse:
    fm = np.full(n_cells, np.nan, dtype=np.float32)
    sel = (val_ta == a) & np.isfinite(val_tws)
    fm[val_cc[sel]] = val_tws[sel]
    AF[a] = fm - mu_fit
...
# trendex
# per-cell linear trend from fit period - long-form bincount (memory-safe)
tp_ = fitp[['cc','t_abs','TWS_t']].dropna()
ccv = tp_['cc'].values.astype(np.int64)
tv_ = tp_['t_abs'].values.astype(np.float64)
yv_ = tp_['TWS_t'].values.astype(np.float64)
cnt_  = np.bincount(ccv, minlength=n_cells).astype(np.float64)
sum_t = np.bincount(ccv, weights=tv_, minlength=n_cells)
sum_y = np.bincount(ccv, weights=yv_, minlength=n_cells)
sum_tt = np.bincount(ccv, weights=tv_*tv_, minlength=n_cells)
sum_ty = np.bincount(ccv, weights=tv_*yv_, minlength=n_cells)
with np.errstate(divide='ignore', invalid='ignore'):
    tbar_c = sum_t / cnt_
    sxx = sum_tt - sum_t*sum_t / cnt_          # sum (t - tbar)^2 per cell
    sxy = sum_ty - sum_t*sum_y / cnt_          # sum (t - tbar)(y - ybar) per cell
    beta_c = np.where((sxx > 100) & (cnt_ >= 24), sxy / np.where(sxx > 0, sxx, 1), 0.0)
beta_c = beta_c.astype(np.float32)
tbar_c = np.nan_to_num(tbar_c, nan=0.0).astype(np.float64)

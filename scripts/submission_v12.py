"""SUBMISSION V12 — the surgical era-Dhat factorial (v11 post-mortem rebuild).

Board: v10b = 0.699997215 (BEST). v11b = 0.710571063 (REGRESSION, 3 bundled changes).

v12_attribution.py (val 2013-15, 140,573 real masked rows) decomposed v11's bundle:
  era-surgical (pred line only)   : -0.0153  GENUINE GAIN (h4-h5: -0.04; h2: +0.005)
  honest-LOO recalibration        : +0.0028  PURE DAMAGE (unprincipled 'principle')
  cov-slow student                : +0.0066  PURE DAMAGE (std 0.21, true r~0.08, poisons H/R)
Split-decode: public masked rows are ALL h2/h3 (era ~neutral); private 2017 block is
h4-h7 (era -0.03/-0.04). Era is a PRIVATE-ROUND investment costing ~nothing on public.

k0 insight: static Dtil mixes early (small-D) and late (big-D) anchors -> systematic D
bias at k0 rows in BOTH eras; era-inclusive Dt0/Dt1 (own anchor weight 0.32-0.49) fixes it.
v11's LOO-Dt0 at own anchors was the WORST choice (removed the best information).

Variants (all else byte-identical to v10b; calibrate/init/k0-training FROZEN):
  ctrl : exact v10b rebuild (offline fidelity check vs submitted CSV)
  v12a : masked pred line uses era Dtil (tau=12)  [private investment]
  v12b : k0-B Dt0/Dt1 use era-inclusive Dtil      [public lever]
  v12c : both (additive by construction: disjoint row classes)
Student and honest-recalibration are RETIRED (measured dead on val).
"""
...
# ---------------- cov fields + anchors (v10 verbatim) ----------------
...
# ---------------- Dhat: static + era; weights LOO-fit (v10/v11 recipes) ----------------
...
# LOO diagnostics (context)
for lbl, f in [('static', lambda a: w1s*dhat_static_loo(a) + w2s*S + w3s*trendex(a)),
               ('era   ', lambda a: w1e*dhat_era(a, excl=a) + w2e*S + w3e*trendex(a))]:
    res = []
    for a in anchors:
        d_ = f(a); ok = np.isfinite(d_) & np.isfinite(AF[a])
        res.append(np.sqrt(np.nanmean((d_[ok]-AF[a][ok])**2)))
    print(f"  {lbl} D-tilde LOO RMSE at anchors: {np.sqrt(np.mean(np.array(res)**2)):.4f}")

# ---------------- Kalman: v10b recipe FROZEN; pred Dtil parameterized ----------------
LAM_F = 0.84
...
def kalman_predict(phi_f, Wd, pred_dtil):
    ...
    for a in anchors:
        fa = AF[a] - Dtil_static(a)          # v10b init: static-full
        ...
        if len(sel0):
            pred[sel0] = mu_c[cc_t[sel0]] + pred_dtil(a+1)[cc_t[sel0]] + phi_f*x[cc_t[sel0]]
        for k in range(1, 9):
            m = a + k
            ...
            pred[sel] = mu_c[cc_t[sel]] + pred_dtil(tm)[cc_t[sel]] + x2[cc_t[sel]]
    return pred
...
# ---------------- k=0 model B: v8 two-comp (v10 verbatim; trained ONCE) ----------------
...
# ---- masked predictors ----
print("\nmasked Kalman runs (static pred = v10b core)...", flush=True)
p74_s = kalman_predict(0.74, W_pool, Dtil_static)
p80_s = kalman_predict(0.80, W_pool, Dtil_static)
p_ens_static = 0.5*p74_s + 0.5*p80_s
print("masked Kalman runs (era pred = v12a axis)...", flush=True)
p74_e = kalman_predict(0.74, W_pool, Dtil_era)
p80_e = kalman_predict(0.80, W_pool, Dtil_era)
p_ens_era = 0.5*p74_e + 0.5*p80_e

# ---- k0-B in two flavors ----
print("\nk0-B (static Dcache = v10b)...", flush=True)
k0B_static = build_k0B(Dtil_static)
print("k0-B (era-inclusive Dcache = v12b axis)...", flush=True)
k0B_era = build_k0B(Dtil_era)

# ---- CONTROL: exact v10b rebuild ----
print("\n--- ctrl: exact v10b rebuild (fidelity check) ---", flush=True)
pred_ctrl = np.empty(len(test), dtype=np.float64)
pred_ctrl[unm] = k0B_static[unm]
pred_ctrl[msk] = p_ens_static[msk]
pred_ctrl = np.where(np.isnan(pred_ctrl), mu_c[cc_t], pred_ctrl)
pred_ctrl = smooth_rows(pred_ctrl, msk, GAU2)
try:
    disk = pd.read_csv(f'{DL}/submission_v10b.csv')['Target'].values.astype(np.float64)
    d = pred_ctrl - disk
    print(f"FIDELITY vs submitted v10b: corr={np.corrcoef(pred_ctrl, disk)[0,1]:.6f} "
          f"mean|d|={np.abs(d).mean():.6f} max|d|={np.abs(d).max():.6f} "
          f"exact-rows={int((np.abs(d)<1e-6).sum())}/{len(d)}")
except Exception as e:
    print(f"FIDELITY check skipped: {e}")

# ---- variants ----
print("\n--- v12a: masked pred = era Dtil (private investment) ---", flush=True)
assemble(p_ens_era, k0B_static, 'v12a')
print("--- v12b: k0-B Dt0/Dt1 = era-inclusive (public lever) ---", flush=True)
assemble(p_ens_static, k0B_era, 'v12b')
print("--- v12c: both axes ---", flush=True)
assemble(p_ens_era, k0B_era, 'v12c')

# ---------------- verification ----------------
print("\n=== verification ===", flush=True)
base = pd.read_csv(f'{DL}/submission_v10b.csv')['Target'].values
pub = ta < (int(test_months.min()) + 12)
for tag in ['v12a','v12b','v12c']:
    ...
    for wname, wmask in [('public', pub), ('private', ~pub)]:
        for cname, cmask in [('k0', unm), ('msk', msk)]:
            mm = wmask & cmask
            if mm.sum():
                print(f"  {tag} {wname} {cname}: n={int(mm.sum()):6d} mean|d|={np.abs(d[mm]).mean():.4f} rms(d)={np.sqrt((d[mm]**2).mean()):.4f}")
    print(f"  {tag}: corr vs v10b={np.corrcoef(o['Target'].values, base)[0,1]:.5f} "
          f"range [{o['Target'].min():.3f}, {o['Target'].max():.3f}]")
print("\nDONE.")
